...
function draw() {
  ...
  fill(0);
  textSize(16);
  text("Determine the rate of change using rise over run (y₂ - y₁) / (x₂ - x₁)", 50, 30);
  text("A sequence starts at term 3 with value 2 and reaches term 7 with value 10.", 50, 50);
  ...
  fill(255, 0, 0);
  noStroke();
  let x1 = originX + (3 * gridWidth / 7);
  let y1 = originY - (2 * gridHeight / 12);
  let x2 = originX + (7 * gridWidth / 7);
  let y2 = originY - (10 * gridHeight / 12);
