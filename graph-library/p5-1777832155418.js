...
function draw() {
  ...
  textSize(16);
  fill(0);
  text("Calculate the rate of change using the formula: (y₂ - y₁) / (x₂ - x₁)", 20, 30);
  text("The pattern goes from (0, 2) to (6, 14). Find the rate of change.", 20, 50);
  ...
  fill(255, 0, 0);
  noStroke();
  ellipse(originX, originY - 1 * gridSize, 8, 8); // Point (0, 2)
  ellipse(originX + 6 * gridSize, originY - 7 * gridSize, 8, 8); // Point (6, 14)
  
  // Label points
  fill(0);
  textAlign(CENTER);
  text("(0, 2)", originX - 20, originY - 1 * gridSize);
  text("(6, 14)", originX + 6 * gridSize + 25, originY - 7 * gridSize);
  
  // Draw line connecting points
  stroke(0, 0, 255);
  strokeWeight(2);
  line(originX, originY - 1 * gridSize, originX + 6 * gridSize, originY - 7 * gridSize);
