...
function draw() {
  ...
  textSize(16);
  fill(0);
  text("Determine the rate of change using rise over run: (y₂ - y₁) / (x₂ - x₁)", 20, 30);
  text("The sequence begins at (0, 5) and ends at (4, 13). What is the rate?", 20, 50);
  ...
  fill(255, 0, 0);
  noStroke();
  ellipse(originX, originY - 2.5 * gridSize, 8, 8); // Point (0, 5)
  ellipse(originX + 4 * gridSize, originY - 6.5 * gridSize, 8, 8); // Point (4, 13)
  
  // Label points
  fill(0);
  textAlign(CENTER);
  text("(0, 5)", originX - 20, originY - 2.5 * gridSize);
  text("(4, 13)", originX + 4 * gridSize + 25, originY - 6.5 * gridSize);
  
  // Draw line connecting points
  stroke(0, 0, 255);
  strokeWeight(2);
  line(originX, originY - 2.5 * gridSize, originX + 4 * gridSize, originY - 6.5 * gridSize);
